...
x = ones(1,n_k+1);          % state with v=0
x_v = ones(n_sample,n_k+1); % state with v~N(0,1)
...
plot(0:n_k,x','LineWidth',1,'color',[0,0,0]+0); 
...
plot(0:n_k,x_v','LineWidth',0.2,'color',[0,0,0]+0.7); 
plot(0:n_k,x','LineWidth',1,'color',[0,0,0]+0); 
xlabel('$k$','Fontsize',16,'Interpreter','latex')
ylabel('$x_k$','Fontsize',16,'Interpreter','latex')
ylim([-x_max,x_max]);
